...
{
            float anguloDeseado = Mathf.Atan2(direccion.x, 0f) * Mathf.Rad2Deg + Camera.main.transform.eulerAngles.y; // changed the direction.z to 0f
            float angulo = Mathf.SmoothDampAngle(transform.eulerAngles.y, anguloDeseado, ref velocidadGiro, tiempoGiro);
            transform.rotation = Quaternion.Euler(0f, angulo, 0f);

            Vector3 direccionMovimiento = transform.forward; // changed this to always use the character's forward
            cuerpo.Move(direccionMovimiento * velocidadHorizontal * Time.deltaTime * vertical); // multiply by vertical to control forward/backward movement
}
